...
The solution’s complexity is very low; besides, the smartphones and beacons no additional hardware or infrastructure is required. This reduces the initial and ongoing costs. The whole localization takes place on the users' smartphones. Compared to other approaches, the location estimation is very robust. During the evaluation, a user’s stationary location, with a mean error of 2.29\,m was achieved. Furthermore, a user’s path in the test environment could be tracked. In addition, the solution's algorithm detects wrong position estimations to recover.}

\newcommand{\ausgabedatum}{01.11.2014}
\newcommand{\abgabedatum}{28.04.2015}
